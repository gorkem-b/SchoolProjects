...
namespace Unity.FPS.Gameplay
{
    ...
    public class PlayerCharacterController : MonoBehaviour
    {
        ...
        [Tooltip("Minimum airtime required before any fall damage can be applied")]
        public float MinAirTimeForFallDamage = 0.35f;

        [Tooltip("Minimum vertical drop required before any fall damage can be applied")]
        public float MinFallDistanceForDamage = 3.5f;

        [Tooltip("Minimum vertical drop required before fall damage can apply when landing on ramps or stairs")]
        public float MinSlopeLandingFallDistanceForDamage = 10f;
        ...
        float m_AirborneStartTime = Mathf.NegativeInfinity;
        float m_AirborneStartHeight;
        ...
        void Update()
        {
            ...
            {
                m_AirborneStartTime = Time.time;
                m_AirborneStartHeight = transform.position.y;
            }
            ...
            {
                // Fall damage
                float airborneDuration = Time.time - m_AirborneStartTime;
                float verticalDrop = m_AirborneStartHeight - transform.position.y;
                float landingSlopeAngle = Vector3.Angle(m_GroundNormal, Vector3.up);
                bool landedOnSlope = landingSlopeAngle > 4f;
                float requiredFallDistance = landedOnSlope
                    ? Mathf.Max(MinFallDistanceForDamage, MinSlopeLandingFallDistanceForDamage)
                    : MinFallDistanceForDamage;
                float fallSpeed = -Mathf.Min(CharacterVelocity.y, m_LatestImpactSpeed.y);
                float fallSpeedRatio = (fallSpeed - MinSpeedForFallDamage) /
                                       (MaxSpeedForFallDamage - MinSpeedForFallDamage);
                bool shouldApplyFallDamage = RecievesFallDamage
                    && fallSpeedRatio > 0f
                    && airborneDuration >= MinAirTimeForFallDamage
                    && verticalDrop >= requiredFallDistance;
                ...
                m_AirborneStartTime = Mathf.NegativeInfinity;
                m_AirborneStartHeight = transform.position.y;
            }
