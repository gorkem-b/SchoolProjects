...
namespace Unity.FPS.Game
{
    ...
    public class WeaponController : MonoBehaviour
    {
        ...
        [Tooltip("Damage multiplier applied to projectiles fired by this weapon")]
        ...
        public event Func<bool> ShootPermissionQuery;
        ...
        bool CanProcessShootRequest()
        {
            if (ShootPermissionQuery == null)
                return true;

            Delegate[] delegates = ShootPermissionQuery.GetInvocationList();
            for (int i = 0; i < delegates.Length; i++)
            {
                if (delegates[i] is Func<bool> query && !query.Invoke())
                    return false;
            }
