/**
 * @file TelecomBill.c
 * @brief Manages a simple telecom billing system with in-memory records.
 *
 * Provides functions to add, list, update, query, and delete customer billing
 * records while calculating total usage charges based on minutes consumed.
 */
#include <stdio.h>
#include <string.h>
...
void addRecord() {
    if (customerCount < 100) {
        ...
        customers[customerCount].totalBill = customers[customerCount].usage * 0.1f;
        customerCount++;
...
void modifyRecord(char phoneNumber[]) {
    for (int i = 0; i < customerCount; i++) {
        if (strcmp(customers[i].phoneNumber, phoneNumber) == 0) {
            ...
            customers[i].totalBill = customers[i].usage * 0.1f;
