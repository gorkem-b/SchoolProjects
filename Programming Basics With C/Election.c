/**
 * @file Election.c
 * @brief Implements a simple command-line voting system that manages candidate registration,
 *        collects votes from multiple voters, and determines the election outcome.
 *
 * The program allows the user to:
 * - Specify the number of candidates (up to MAX_C - 1) and assign each a unique symbol.
 * - Register candidate names while ensuring no duplicate selection of predefined symbols.
 * - Gather votes from a configurable number of voters, validating each choice.
 * - Display the election winner if a unique candidate receives the highest votes;
 *   otherwise, it reports the absence of a majority winner.
 */
#include <stdio.h>
#include <string.h>
...
int main() {
    ...
    if (candidateCount >= MAX_C) {
        printf("Number of candidates cannot be greater than 10. Terminating the program\n\n");
        return 0;
    }
